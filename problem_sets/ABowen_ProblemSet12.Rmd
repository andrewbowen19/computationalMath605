...
This dataset looks much more linear. Let's run our linear model on this data and the interpret our `summary`
...
Using R's built-in `predict` function, we can pass our `linModelRaised` as well as a dataframe of our new `TotExp` values
```{r}
# 
...
Let's look at and interpret our summary stas on this model
```{r}
summary(mlm)
```
