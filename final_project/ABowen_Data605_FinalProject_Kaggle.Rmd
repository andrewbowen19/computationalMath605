...
for (i in colnames(corTrain)){
  for (j in colnames(corTrain)){
    if (i != j){
      test <- cor.test(corTrain[,c(i)], corTrain[,c(j)], conf.level=0.8)
      print(test)
    }
  }
}
...
For each combination of variables tested, we have p-values greater than our significance level ($\alpha = 0.2$). This indicates that we can not reject our null hypothesis that the true correlation between variables is equal to 0. 
...
Our determinant is **not** equal to 0, so we should be able to invert our correlation matrix
...
We can check this assumption with our QQ-plot. The plot is generally linear in the middle of our residuals, but we can test this with a simple histogram and a [Shapiro Test](https://en.wikipedia.org/wiki/Shapiro–Wilk_test)
```{r}
hist(mylm$residuals)
```
